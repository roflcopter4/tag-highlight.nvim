...
#define COUNT()            $get_fd_count((1), false)
#define INC_COUNT()        $get_fd_count((1), true)

extern int $get_fd_count(int fd, bool inc);

#define INTERN __attribute__((__visibility__("hidden"))) extern

INTERN mpack_obj    *$nvim_api_generic_call(bool blocking, const bstring *fn, const bstring *fmt, ...);
INTERN mpack_obj    *$nvim_api_special_call(bool blocking, const bstring *fn, mpack_obj *pack, int count);
INTERN mpack_retval  m_expect_intern(mpack_obj *root, mpack_expect_t type) __aWUR;

#undef INTERN
#define generic_call $nvim_api_generic_call
#define special_call $nvim_api_special_call
