...
__attribute__((__constructor__(400))) static void
mutex_init(void)
{
...
#else
# define FIX_FOPEN_MODE(OLD, NEW) char const *const NEW = (OLD)
#endif
...
ND int
safe_open(char const *const filename, int const flags, UNUSED int const mode)
{
#ifdef _WIN32
...
ND int
safe_open_fmt(int const flags, UNUSED int const mode,
              char const *const restrict fmt, ...)
{
      va_list va;
...
bstring *
get_command_output(char const        *command,
...
int
win32_start_process_with_pipe(char const *exe,
                              char *argv,
...
bstring *
_win32_get_command_output(char *argv, bstring const *input, int *status)
{
      PROCESS_INFORMATION pi;
      HANDLE handles[2];
      DWORD written, st;
      ...
      if (!WriteFile(handles[WRITE_FD], input->data, input->slen, &written, NULL) ||
          written != input->slen)
