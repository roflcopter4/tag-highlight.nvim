...
UNUSED static void braindead(token_t *tok);
...
retry:
      ...
      case CXCursor_CXXBaseSpecifier:
      {
            ...
            if (bdata->ft->id == FT_CXX) {
                  ...
                  CXCursor newcurs = clang_getCursorDefinition(tok->cursor);
      ...
      case CXCursor_MemberRefExpr: {
            ...
            if (bdata->ft->id != FT_C && deftype.kind == CXType_Unexposed) {
                  cursor = clang_getCursorReferenced(tok->cursor);
                  goto retry;
            } else {
                  ADD_CALL(CTAGS_MEMBER);
            }
      ...
      case CXCursor_TemplateTemplateParameter:
            ADD_CALL(EXTENSION_NT_TEMPLATE_PARAM);
            break;
...
braindead(token_t *tok)
{
        ...
        fprintf(dump_fp, "Have (%s): (`%s`  TYPE  `%s`) : TK = `%s` **** references `%s`  -->  `%s`   -->  `%s`   -->   %u:[%u, %u]\n",
