...
if (NOT LIBCLANG_LLVM_CONFIG_EXECUTABLE)
    ...
    if (LIBCLANG_LLVM_CONFIG_EXECUTABLE)
        if (UNIX)
            execute_process(COMMAND which "${LIBCLANG_LLVM_CONFIG_EXECUTABLE}" OUTPUT_VARIABLE _LIBCLANG_RESOLVED_LLVM_CONFIG_EXECUTABLE1 OUTPUT_STRIP_TRAILING_WHITESPACE)
            execute_process(COMMAND realpath "${_LIBCLANG_RESOLVED_LLVM_CONFIG_EXECUTABLE1}" OUTPUT_VARIABLE LIBCLANG_LLVM_CONFIG_EXECUTABLE OUTPUT_STRIP_TRAILING_WHITESPACE)
            unset(_LIBCLANG_RESOLVED_LLVM_CONFIG_EXECUTABLE1)
        endif ()
        message(STATUS "llvm-config executable found: ${LIBCLANG_LLVM_CONFIG_EXECUTABLE}")
    endif ()
endif ()

if (NOT LIBCLANG_CFLAGS)
    if (NOT LIBCLANG_LLVM_CONFIG_EXECUTABLE)
        message(FATAL_ERROR "Could NOT find llvm-config executable and LIBCLANG_CFLAGS is not set ")
    endif ()
    execute_process(COMMAND ${LIBCLANG_LLVM_CONFIG_EXECUTABLE} --cxxflags OUTPUT_VARIABLE LIBCLANG_CFLAGS OUTPUT_STRIP_TRAILING_WHITESPACE)
    if (NOT LIBCLANG_CFLAGS)
        find_path(LIBCLANG_CFLAGS_HACK_CMAKECACHE_DOT_TEXT_BULLSHIT clang-c/Index.h HINTS ${LIBCLANG_ROOT_DIR}/include NO_DEFAULT_PATH)
        if (NOT EXISTS ${LIBCLANG_CFLAGS_HACK_CMAKECACHE_DOT_TEXT_BULLSHIT})
            find_path(LIBCLANG_CFLAGS clang-c/Index.h)
            if (NOT EXISTS ${LIBCLANG_CFLAGS})
                message(FATAL_ERROR "Could NOT find clang include path. You can fix this by setting LIBCLANG_CFLAGS in your shell or as a cmake variable.")
            endif ()
        else ()
            set(LIBCLANG_CFLAGS ${LIBCLANG_CFLAGS_HACK_CMAKECACHE_DOT_TEXT_BULLSHIT})
        endif ()
        set(LIBCLANG_CFLAGS "-I${LIBCLANG_CFLAGS}")
    endif ()
...
message("-- Using Clang version ${LIBCLANG_VERSION_STRING} from ${LIBCLANG_LIBDIR} with CXXFLAGS ${LIBCLANG_CXXFLAGS} and CFLAGS ${LIBCLANG_CFLAGS}")
